...
## Trigger / Frequency
- Trigger: Authorized operator submits the account-closure route or explicitly runs the closure dev script.
- Frequency: On demand.
...
3. AGM signs the `accountManagementRequests.accountClose` payload and submits `POST /gw/api/v1/accounts/close` to IBKR.
...
- Local execution output when the dev script is used.
...
- Dev script is dry-run by default and requires `--execute` for an external state change.
...
- `agm-api/dev/close_natalia_account.py`
