...
3. If one or more sanctions files are unavailable, the process logs the gap and includes the unavailable-list detail in the returned summary.
4. If all three sanctions lists are available and unchanged, or if no confirmed delta can be established, the process exits early and returns a compact “skipped” result.
5. Only when at least one list has a confirmed additions/removals delta does the API load accounts, IBKR details backup data, account-contact links, contacts, and existing contact screenings.
...
12. GitHub Actions sends a success or failure email summarizing the run, including coverage counts, contacts with and without sanctions-list matches, per-list match counts, contact ids associated with screening errors, and sanctions-availability warnings from the API response when present.
...
- Sanctions file unavailable: process logs the missing list and skips unless another list has a confirmed additions/removals delta; the unavailable-list detail remains in the response summary.
...
- API response summary fields for coverage, sanctions matches, screening-error contact ids, `sanctions_unavailable_lists`, and `summary_messages` when present

## Related Code / Pages / Routes
- Entry surfaces: `agm-api/.github/workflows/daily_screening.yaml`, `agm-api/src/app/tools/private/actions.py`
- Supporting modules: `agm-api/src/components/clients/contacts.py`, `compliance/processes/15b-compliance-reference-data-refresh.md`, `compliance/processes/16-contact-screening-and-aml-risk-assessment.md`
