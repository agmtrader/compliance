...
7. Account-level profile-document support compares local OCR documents to profile data. ID OCR is checked against linked contact and holder names. Source-of-Wealth OCR is checked against application/IBKR net worth, liquid net worth, and annual income ranges. Application and IBKR residence addresses are compared with each other and checked against ID and Proof-of-Address OCR. POA is primary residence evidence; an ID address can support residence when no POA is available, while an ID containing no address is inconclusive rather than conflicting. A SOW document is `supported` only when at least one extracted financial amount supports a declared profile value and no clear comparable financial conflict exists. CRC and USD amounts are compared after conversion using `SOW_CRC_PER_USD`, defaulting to `500` CRC per USD unless overridden by the operator; when an extracted amount has no field-level currency, the script can infer CRC or USD from document-wide currency context. Source categories are retained as context, but source-only support, ambiguous periods, unsupported currency pairs, tiny/noisy income artifacts, or otherwise unsafe amount comparisons are `cannot_compare`; clear converted/comparable financial mismatches are `not_supported`. Bank or brokerage statement balances can support liquid net worth when they are high enough, but balances below the declared liquid-net-worth range are treated as insufficient evidence rather than contradictions. Missing required local evidence produces `missing_evidence`; unsafe comparisons produce `cannot_compare`; conflicting evidence produces `not_supported`; and all three ID, SOW, and address checks must support the profile for the combined row to be `supported`. IBKR/application financial and profile mismatch states remain separate columns and are not directly copied into this document-support state.
