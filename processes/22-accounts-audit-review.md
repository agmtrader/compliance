...
6. Source-of-Wealth support remains driven by comparable financial evidence, not by category presence alone. The dashboard attempts keyword-and-amount extraction directly from stored OCR text, compares liquid-net-worth candidates against the application and IBKR liquid-net-worth ranges, separately checks whether current NAV fits within the declared liquid net worth, converts USD and CRC using the same `500 CRC/USD` working rate used by the Python audit, and treats ambiguous currencies or source-only evidence as `cannot_compare`.
